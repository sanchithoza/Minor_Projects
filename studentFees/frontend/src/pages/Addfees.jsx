...
function Addfees(props) {
  ...
  useEffect(() => {
    ...
    if (query) {
      ...
      axios
        .post("http://localhost:7000/getStudentfees", {
          _id: id,
        })
        .then(async (response) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const fileupload = await axios.post(
      "http://localhost:7000/uploadfees",
      formData
    );
    ...
    if (fileupload.data.code == 200) {
      ...
      const response = await axios.post(
        "http://localhost:7000/AddStudentfees",
        data
      );
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    axios
      .patch(`http://localhost:7000/updateStudentfees/${id}`, record)
      .then((response) => {
        console.log(response);
    ...
    <div className="container">
      <div className="row bg-primary p-3">
        <div className="col-12 text-center text-white">
          <h1 className="display-5">Add fees Record</h1>
        </div>
      </div>
      <form
